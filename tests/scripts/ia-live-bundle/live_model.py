...
Generates, from one definition:
  ...
  data_dictionary.yaml         app catalog (RP layout)
  ia_ontology.ttl              OWL/RDFS ontology for n10s (classes = labels, object props = rel types)
...
# ============================================================================
# 7. Emit ia_ontology.ttl for n10s  (paste into live_model.py after section 6)
# ============================================================================
# Loaded with:  CALL n10s.onto.import.fetch("file:///ontology/ia_ontology.ttl", "Turtle")
# n10s turns owl:Class -> (:Class), owl:ObjectProperty -> (:Relationship) with
# DOMAIN/RANGE edges, owl:DatatypeProperty -> (:Property) with a DOMAIN edge.
# Local names are the exact Neo4j labels / rel types / property keys, so the
# ontology reconciles 1:1 against db.labels() / db.relationshipTypes().

ONTO_NS = "http://ia.gov.sa/ontology#"
XSD = {"String": "xsd:string", "Long": "xsd:long", "Double": "xsd:double",
       "Boolean": "xsd:boolean", "DateTime": "xsd:dateTime", "Date": "xsd:date"}

def _ttl_str(s: str) -> str:
    return '"' + s.replace("\\", "\\\\").replace('"', '\\"').replace("\n", " ") + '"'

def write_ontology(path: str = "ia_ontology.ttl"):
    out = [
        "# IA Knowledge Graph ontology - GENERATED by live_model.py. Do not hand-edit.",
        f"@prefix ia:   <{ONTO_NS}> .",
        ...
        "",
        "ia: a owl:Ontology ;",
        '    rdfs:label "IA Insurance Knowledge Graph" ;',
        f"    rdfs:comment {_ttl_str('Generated ' + str(dt.date.today()) + ' from the live Neo4j schema.')} .",
        "",
        "# ---- Classes (node labels) ----",
    ]
    for lbl, n in NODES.items():
        comment = f"{n['desc']} Key: {n['key']}."
        if n.get("rollups"):
            comment += " Rollups: " + ", ".join(n["rollups"]) + "."
        out += [f"ia:{lbl} a owl:Class ;",
                f"    rdfs:label {_ttl_str(lbl)} ;",
                f"    rdfs:comment {_ttl_str(comment)} .", ""]

    out.append("# ---- Object properties (relationship types) ----")
    for f_, t, to, p, note in RELS:
        comment = (note.rstrip(".") + ".") if note else f"{f_} {t.lower().replace('_', ' ')} {to}."
        if p:
            comment += " Edge properties: " + ", ".join(f"{k} ({v.lower()})" for k, v in p.items()) + "."
        out += [f"ia:{t} a owl:ObjectProperty ;",
                f"    rdfs:label {_ttl_str(t)} ;",
                f"    rdfs:domain ia:{f_} ;",
                f"    rdfs:range ia:{to} ;",
                f"    rdfs:comment {_ttl_str(comment)} .", ""]

    out.append("# ---- Datatype properties (node properties) ----")
    # one property URI per key; a key shared by several labels gets several rdfs:domain triples
    props: dict[str, dict] = {}
    for lbl, n in NODES.items():
        for k, t in n["props"].items():
            entry = props.setdefault(k, {"domains": [], "types": set(), "descs": []})
            entry["domains"].append(lbl)
            entry["types"].add(t)
            d = PROP_DESC.get(f"{lbl}.{k}")
            if d:
                entry["descs"].append(f"{lbl}: {d}")
    for k, e in props.items():
        rng = XSD[next(iter(e["types"]))] if len(e["types"]) == 1 else "rdfs:Literal"
        out.append(f"ia:{k} a owl:DatatypeProperty ;")
        out.append(f"    rdfs:label {_ttl_str(k)} ;")
        for d in e["domains"]:
            out.append(f"    rdfs:domain ia:{d} ;")
        out.append(f"    rdfs:range {rng} ;")
        pii_on = [l for l in e["domains"] if k in PII.get(l, [])]
        comment = " | ".join(e["descs"]) if e["descs"] else k.replace("_", " ")
        if pii_on:
            comment += " PII on " + ", ".join(pii_on) + " - redacted on output."
        out.append(f"    rdfs:comment {_ttl_str(comment)} .")
        out.append("")

    with open(path, "w", encoding="utf-8") as f:
        f.write("\n".join(out))
    return len(NODES), len(RELS), len(props)



if __name__ == "__main__":
    write_questions(); write_dictionary(); write_modelfile(); write_app_dictionary()
    n_cls, n_rel, n_prop = write_ontology()
    ...
    print(f"ontology: classes={n_cls} object_props={n_rel} datatype_props={n_prop} -> ia_ontology.ttl")